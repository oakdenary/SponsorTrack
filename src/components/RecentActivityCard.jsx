            ...
            <div className="flex flex-col flex-1 overflow-y-auto pr-2 relative">
                ...
                {activities.map((activity) => (
                    <div key={activity.id} className="flex gap-4 items-start py-3 cursor-pointer group relative z-10">
                        <div className={`mt-0.5 flex items-center justify-center min-w-7 h-7 rounded-full shadow-sm z-10 transition-transform group-hover:scale-110 ${activity.color} dark:ring-[#111]`}>
                            {activity.icon}
                        </div>
                        <div className="flex flex-col bg-white/50 dark:bg-transparent group-hover:bg-zinc-50 dark:group-hover:bg-zinc-800/50 rounded-xl px-3 py-1.5 -ml-2 transition-colors w-full border border-transparent group-hover:border-zinc-100 dark:group-hover:border-zinc-800/50">
                            <span className="text-zinc-800 dark:text-zinc-200 font-semibold text-sm leading-tight tracking-tight">{activity.title}</span>
                            <span className="text-zinc-400 dark:text-zinc-500 text-xs mt-0.5 font-medium">{activity.time}</span>
                        </div>
                    </div>
                ))}
            </div>
