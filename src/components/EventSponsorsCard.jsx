...
export function EventSponsorsCard() {
    ...
    return (
        <div className="bg-white rounded-[1.5rem] p-6 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] border border-zinc-100 flex flex-col h-full w-full relative">
            <div className="flex justify-between items-center mb-4">
                <div>
                    <h2 className="text-zinc-900 font-bold text-lg tracking-tight">Event Sponsors</h2>
                    <p className="text-xs text-zinc-500 font-medium mt-0.5">Categorized domains</p>
                </div>
                <div className="relative">
                    <button
                        onClick={() => setIsOpen(!isOpen)}
                        className="flex items-center gap-1.5 text-zinc-600 hover:text-zinc-900 font-semibold text-xs border border-zinc-200 rounded-full px-3 py-1.5 hover:bg-zinc-50 transition-colors"
                    >
                        {selectedEvent} <ChevronDown className="w-3 h-3" />
                    </button>

                    {isOpen && (
                        <div className="absolute right-0 top-full mt-2 w-40 bg-white border border-zinc-100 rounded-xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] overflow-hidden z-20">
                            {Object.keys(eventData).map(evt => (
                                <button
                                    key={evt}
                                    onClick={() => { setSelectedEvent(evt); setIsOpen(false); }}
                                    className="w-full text-left px-4 py-2.5 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 hover:text-zinc-900 transition-colors"
                                >
                                    {evt}
                                </button>
                            ))}
