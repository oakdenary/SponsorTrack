    ...
    return (
        <div className="bg-white rounded-[1.5rem] p-5 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.1)] border border-zinc-100 flex flex-col h-full w-full overflow-hidden">
            <div className="flex justify-between items-center mb-3 shrink-0">
                <div>
                    <h2 className="text-zinc-900 font-bold text-base tracking-tight">Timeline</h2>
                    <p className="text-[11px] text-zinc-500 font-medium mt-0.5">{format(currentDate, "MMMM yyyy")} Follow-ups</p>
                </div>
                    ...
                    {days.map((day, idx) => {
                        ...
                        return (
                            <div key={idx} className="flex flex-col items-center gap-1.5 cursor-pointer shrink-0">
                                <span className={`text-[10px] font-bold uppercase tracking-wider ${isToday ? 'text-zinc-900' : 'text-zinc-400'}`}>
                                    {format(day, 'E').slice(0, 2)}
                                </span>
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${isToday ? 'bg-zinc-900 text-white' : 'text-zinc-700 hover:bg-zinc-100'}`}>
                                    {format(day, 'd')}
                                </div>
                                {followUp ? (
                ...
                <div className="flex gap-3 overflow-x-auto overflow-y-hidden pb-2 pt-1 px-1 -mx-1 snap-x scrollbar-hide flex-1 min-h-0">
                    {days.map((day, idx) => {
                        ...
                        return (
                            <div
                                key={idx}
                                className={`flex-shrink-0 flex flex-col p-3 w-[140px] rounded-2xl cursor-pointer transition-all snap-start shadow-sm border ${isToday ? 'bg-zinc-900 text-white border-zinc-900' : 'bg-zinc-50 hover:bg-zinc-100 border-zinc-200/60'}`}
                            >
                                <div className="flex justify-between items-center mb-3">
                                    <span className={`text-[10px] font-bold uppercase tracking-wider ${isToday ? 'text-zinc-400' : 'text-zinc-500'}`}>
                                        {format(day, 'E')}
                                    </span>
                                    <span className={`text-lg font-bold ${isToday ? 'text-white' : 'text-zinc-900'}`}>
                                        {format(day, 'dd')}
                                    </span>
                                </div>
                                <div className="flex flex-col flex-1 min-h-0">
                                    {followUp ? (
                                        <>
                                            <span className={`text-xs font-bold truncate mb-0.5 ${isToday ? 'text-white' : 'text-zinc-800'}`}>
                                                {followUp.sponsor}
                                            </span>
                                            <span className={`text-[10px] font-medium truncate mb-0.5 ${isToday ? 'text-zinc-300' : 'text-zinc-600'}`}>
                                                {followUp.event}
                                            </span>
                                            <span className={`text-[10px] font-semibold mb-2 ${isToday ? 'text-zinc-400' : 'text-zinc-500'}`}>
